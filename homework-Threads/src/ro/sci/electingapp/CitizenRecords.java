...
public class CitizenRecords {
	...
	public void printReport(ArrayList<Citizen> listOfCitizen) {
		if (listOfCitizen.isEmpty()) {
			System.out.println("No citizen has voted yet!\n\n");
		} else {
			HashMap<String, ArrayList<Citizen>> mapOfVotes = new HashMap<>();
			System.out.println("Reports------------");
			mapOfVotes = generateVoteListsOfCitizens(listOfCitizen);
			if (mapOfVotes.get("invalid").isEmpty()) {
				System.out.println("No citizen has broken the rules");
			} else {
	...
	int computeNrOfInvalidVotes(ArrayList<Citizen> listOfCitizen) {
		...
		// listOfCitizen.size()-temp.size() = nr of citizen who votes multiple
		// times
	...
	String computeElectedMayor(HashMap<String, Integer> mayorMap) {
		...
		// check for another candidate with the max number of votes
		int nrOfApearance = 0;
		for (Entry<String, Integer> entry : mayorMap.entrySet()) {
			if (entry.getValue() == maxNrOfVotes) {
				nrOfApearance++;
		...
		if (nrOfApearance > 1) {
			return "!!! " + nrOfApearance + " candidates have the same number of votes.";
		}
		return electedMayor;
	}
